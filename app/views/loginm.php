    ...
    if(isset($_POST['login']))
    {
        ...
        if(!empty($email) && !empty($password)) {
          ...
          if($validate !== false) {
            if(sizeof($validate) > 1) {
             ...
             else echo("<script>window.location.href='/dashboard_member'</script>");
        ...
        if(isset($_COOKIE['club_id'])){
          if(sizeof($cred) > 1) {
            ...
            $log = $auth->ChooseClub('club_members',$cred,$club_id); 
         
            echo("<script>window.location.href='/dashboard_member'</script>");
